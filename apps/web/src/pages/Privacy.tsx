...
const sections: DocSection[] = [
  ...
  },
  {
    ...
    body: (
      <>
        ...
        <p>For every test run we start or refuse: your account, the site's host name, whether it ran as a visitor or on your verified domain, the goal, and each action's type with the label of the control it used (for example "click: Pricing"). Never what was typed, never page text. It exists to investigate abuse and is kept 90 days, even if you delete the run.</p>
        <h3>Feedback and error reports</h3>
        <p>Messages you send us from Send feedback, with the page you were on. When the service fails, we record which part failed and the kind of error, never the content of your request.</p>
  ...
  {
    id: 'extension',
    title: 'What the Chrome extension reads',
    body: (
      <>
        <p>Only the tab you start a test in, and only while the test runs. From that tab it reads a masked outline of the page (buttons, links, headings, visible errors and control labels, with emails, long numbers and typed values masked) and takes up to eight screenshots with form fields hidden.</p>
        <p>It never reads the site's cookies, local storage, session storage or saved passwords, never reads your browsing history or other tabs, and never watches your network requests. An automated test fails our build if the extension's code ever touches those. It talks only to {brand.name}'s API and our database provider.</p>
      </>
    ),
  },
  {
