...
- [ ] **SD-3.1 Ownership test for every route (IDOR suite).** One parametrized pytest that calls each of the 28 routes as user B on user A's run, key, site and comparison and expects 404 or 403. New routes must be added to the list (the test fails on an unlisted route). **GATE.** Hardness 2. Human: No.
