...
### HANDOFF: agent safety item 2 (in progress, branch `claude/practical-hopper-j64i4s`, not on main)
Done: API gate (`safety.py` SOCIAL/COMMERCE/THIRD_PARTY_LOGIN/SEARCH/SIGNED_IN, `persona._enforce` visitor rules and new status `visitor_limit`, `policy.signed_in` start refusal and mid-run stop `signed_in_unverified`, report treats `visitor_limit` as Walkthru's stop, tests in `apps/api/tests/test_policy.py`); extension mirror in `lib/safety.ts` and `lib/execute.ts`.
To finish: 1) extension tests (`tests/snapshot.test.ts` execute test types into an input without `verified`; pass `{ verified: true }` there and add visitor-mode tests: Like button, email field, search box, submit of a non-search form, "Continue with Google" link); 2) add `visitor_limit` to `lib/api.ts` status union, `App.tsx` STATUS_COPY, web `runs.ts` status union + STATUS_LABEL, `ReportView.tsx` tone and a note like the safe_stop one; 3) run API pytest + ruff, extension tsc/lint/test/build, web build/lint; 4) mark item 2 done in docs/agent-safety-plan.md, tasks/todo.md, this file; merge to main.
